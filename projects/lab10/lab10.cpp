...
//simple state_type structure to run fsm
struct state_type {
    std::uint8_t out; //output of state
    std::uint32_t delay; //delay of state
    std::size_t next[8]; //array of states to work with
};

//finite state machine
constexpr state_type fsm[12] = {
...
main()
{
    ...
    // -bit 6 is pf1
    // -bit 7 is pf3
    std::size_t i {go_west}; //index to run through FSM, starting at go_west

    while(1) {
        std::uint8_t output = fsm[i].out;
        e.data(0, 1, 2, 3, 4, 5) = (output & 0b111111); //set 6 LEDS
        f.data(1, 3) = (((output & 0x40) >> 5) | ((output & 0x80) >> 4)); //move bit 6 to bit 1
                                                                          //and bit 7 to bit3

        //e.data(0, 1, 2, 3, 4, 5) = 0x4C;
        vin::sleep(fsm[i].delay);

        i = fsm[i].next[(a.data(2, 3, 4) >> 2)];
    }
}
